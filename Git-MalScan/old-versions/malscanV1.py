...
def scan_with_clamav_low_priority(file_path):
    print(f"[INFO] Scanning {file_path} with ClamAV (nice/low priority)...")
    start = time.time()
    result = subprocess.run(
        ["nice", "-n", "10", "clamscan", "--verbose", file_path],
        capture_output=True,
        text=True
    )
    elapsed = time.time() - start
...
def save_malicious_file(file_path, file_url):
    ...
    if os.path.exists(USB_DIR):
        dest_path = os.path.join(USB_DIR, os.path.basename(file_path))
        ...
        # --- NEW: Write a per-file URL text file ---
        url_txt_path = dest_path + ".url.txt"
...
def get_exe_files_in_repo(repo_full_name, token, path="", depth=0, max_depth=3):
    ...
    try:
        ...
        if response.status_code == 200:
            files = response.json()
            for f in files:
                if f["type"] == "file" and f["name"].endswith(".exe"):
                    exe_files.append(f)
                elif f["type"] == "dir":
                    exe_files.extend(get_exe_files_in_repo(repo_full_name, token, f["path"], depth+1, max_depth))
    except Exception as e:
        if verbose:
            print(f"[ERROR] Failed to fetch {contents_url}: {e}")
    return exe_files

def collect_exe_files(repos):
    exe_files = []
    for repo in repos:
        exe_files.extend(get_exe_files_in_repo(repo["full_name"], GITHUB_TOKEN, "", 0, max_depth))
    return exe_files
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        KEYWORD = input("Enter a keyword to search for (leave blank for all .exe files): ").strip()
        if KEYWORD:
            print(f"[INFO] Using keyword: {KEYWORD}")
        else:
            print("[INFO] No keyword specified, searching all .exe files.")
        ...
        while True:
            ...
            for repo in repos:
                ...
                if repo_full_name in processed_repos:
                    ...
                    continue
                exe_files = get_exe_files_in_repo(repo_full_name, GITHUB_TOKEN, "", 0, max_depth)
                for exe_file in exe_files:
                    file_url = exe_file["download_url"]
                    ...
                    if file_path:
                        clamav_result = scan_with_clamav_low_priority(file_path)
                        if clamav_result:
